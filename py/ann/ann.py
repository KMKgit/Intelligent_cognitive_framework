...
class ann(object):
    ...
    def training(self):
        ...
        model = Sequential()
        #input layer
        model.add(Dense(output_dim = int(self.hidden_unit[0]), input_dim=(self.data_dim)))
        #hidden layer
        for i in range(0, self.hidden_layer):
            if i == self.hidden_layer-1:
                model.add(Dense(int(self.hidden_unit[i])))
            else:
                model.add(Dense(int(self.hidden_unit[i])))
        ...
        json.dump({'ntb':
                    {
                        ...
                        #'precision_score' : precision_score(P, Y_train, average='weighted') # it's not working because of multi-dimension
