...
class ensemble(object):
    ...
    def training(self):
        ...
        methods = [SimpleRNN, LSTM, GRU];
        accuracy = 0
        en_score = 0
        P = []
        en_method = None
        model_name = 'train_'+time.strftime("%Y%m_%d_%H_%M", time.localtime())
        model_path = self.PATH + '/data/' + sys.argv[1] + '/' + model_name + '/' + 'model' + '.h5'

        for method in methods:
          #create model
          model = Sequential()
          #input layer
          model.add(method(output_dim = int(self.hidden_unit[0]), 
                      return_sequences=True, 
                      input_shape=(self.timesteps, self.data_dim)))
          #hidden layer
          for i in range(0, self.hidden_layer):
            if i == self.hidden_layer-1:
                model.add(method(int(self.hidden_unit[i])))
            else:
                model.add(method(int(self.hidden_unit[i]), return_sequences=True))
