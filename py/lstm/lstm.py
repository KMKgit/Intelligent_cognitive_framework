...
class lstm(object):
    ...
    def training(self):
        ...
        model = Sequential()
        #input layer
        model.add(LSTM(output_dim = int(self.hidden_unit[0]), 
                    return_sequences=True, 
                    input_shape=(self.timesteps, self.data_dim)))
        #hidden layer
        for i in range(0, self.hidden_layer):
            if i == self.hidden_layer-1:
                model.add(LSTM(int(self.hidden_unit[i])))
            else:
                model.add(LSTM(int(self.hidden_unit[i]), return_sequences=True))
        ...
        #model.save('./test_py/LSTM/weight.h5')
        del model
        out.close()
