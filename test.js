...
var amazonAddress = 'http://ec2-52-78-84-62.ap-northeast-2.compute.amazonaws.com';
  ...
  fs.writeFile(__dirname + '/data/' + body.apiKey + '/request/' + randNum + '.csv', inpStr, 'utf-8', function(err, result) {
    ...
    execFile('python', exeParam, function(err, stdout, stderr) {
      ...
      fs.readFile(__dirname + '/data/' + body.apiKey + '/request/' + randNum + '.req', 'utf-8', function(err, result) {
        ...
        try {
          execFileSync('rm', [__dirname + '/data/' + body.apiKey + '/request/' + randNum + '.csv']);
          execFileSync('rm', [__dirname + '/data/' + body.apiKey + '/request/' + randNum + '.req']);
        } catch (e) {
          console.error(e);  
        } finally {
          return res.json({
...
MongoClient.connect(mongoUrl, function(err, db) {
  ...
  server.listen(GLOBAL.serverPort, function() {
    ...
    GLOBAL.db.collection(TABLE_NAME.API).find().toArray(function(err, doc){
      if(err) return printError(err);
      console.log(doc);
    });
  });
